...
- `internal/models/webhook.go`
  ...
  - 迁移脚本 `internal/migrations/011_add_webhook_multi_channel.go`：创建 `webhook_settings` 与 `webhook_delivery_stats`，同时回填历史数据并推断渠道类型。
...
| 模块 | 子任务 | 关联文件/位置 | 备注 |
| --- | --- | --- | --- |
| 数据模型 | 拆分 Webhook 主表与 `WebhookSetting` 子表，保留类型字段并实现自动识别 | `internal/models/webhook.go` | 保证向后兼容 |
| 数据迁移 | 创建 `webhook_settings` 与统计表，迁移/回填历史数据 | `internal/migrations/011_add_webhook_multi_channel.go` | 按「建表→回填→初始化统计」顺序 |
| 服务层 | 重构通知服务，引入 Sender 工厂与钉钉实现 | `internal/services/notification.go`、`internal/services/*.go` | 拆分 Sender，便于扩展 |
